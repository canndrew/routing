...
pub mod compat {
    ...
    impl<UID: Uid> Service<UID> {
        /// Create new mock `Service` using the make_current/get_current mechanism to get the
        /// associated `ServiceHandle`.
        pub fn new(
        ...
        /// This method is used instead of dropping the service and creating a new one, which is
        /// the current practice with the real crust.
        ...
        /// Enable listening and responding to peers searching for us. This will allow others
        /// finding us by interrogating the network. Note: `set_service_discovery_listen` is not
        /// yet implemented in mock.
        ...
        /// Start TCP acceptor. Note: mock doesn't currently differentiate between TCP and UDP. As
        /// long as at least one is enabled, the service will accept any incoming connection.
        pub fn start_listening(&self) -> Result<(), CrustError> {
            self.lock().start_listening_tcp(LISTENER_PORT);
            Ok(())
        }

        /// Stops Listener explicitly and stops accepting TCP connections. Note:
        /// `stop_listening` is not yet implemented in mock.
        pub fn stop_listening(&mut self) -> Result<(), CrustError> {
        ...
        /// Connect to a peer using our and their connection infos. The connection infos must be
        /// first prepared using `prepare_connection_info` on both our and their end.
        pub fn connect(
            &self,
    ...
    pub enum Event<UID: Uid> {
        ...
        BootstrapFailed,
        /// Invoked when we are ready to listen for incoming connection. Contains the listening
        /// port.
        ListenerStarted(u16),
...
/// Protocol agnostic address in this case is a regular socket address.
pub type PaAddr = SocketAddr;
